...
export interface FlipCardProps {
   ...
   * Позиционирование и `stopPropagation` на контейнере выполняет `FlipCard`.
   * Можно передать функцию с `{ imageFailed }` для палитры как у оригинальных карт.
   */
  backTopRightSlot?: FlipCardSlot;

  /**
   * Нижняя панель оборота. Позиционирование и `stopPropagation` на контейнере выполняет `FlipCard`.
   */
      ...
      >
        ...
        >
          {hasTopRight && (
            ...
            >
              {backTopRightSlot != null ? resolveSlot(backTopRightSlot) : null}
            </Box>
          )}
          <Typography
            variant="subtitle1"
