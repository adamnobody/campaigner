...
export class MapService {
  ...
  uploadMapImage(mapId: number, file: Express.Multer.File): Map {
    ...
    if (!file.buffer) {
      throw new BadRequestError('Uploaded file buffer is missing');
    }

    if (map.imagePath) {
      const normalizedOldPath = map.imagePath.replace(/^\/+/, '');
      const oldPath = path.join(DATA_DIR, normalizedOldPath);
  ...
  getMarkerByIdOrThrow(markerId: number): MapMarker {
    ...
    if (!marker) {
      throw new NotFoundError('Map marker');
    }
    return marker;
  }
  ...
  getTerritoriesByMapId(mapId: number): MapTerritory[] {
    ...
    return rows.map((row) => ({
      ...row,
      points: parseTerritoryPoints(row.points),
    }));
  }
  ...
  getTerritoryById(territoryId: number): MapTerritory | null {
    ...
    const row = stmt.get(territoryId) as
      | {
          id: number;
          mapId: number;
          name: string;
  ...
  getTerritoryByIdOrThrow(territoryId: number): MapTerritory {
    ...
    if (!territory) {
      throw new NotFoundError('Map territory');
    }
    return territory;
  }
  ...
  updateTerritory(territoryId: number, data: UpdateMapTerritoryData): MapTerritory {
    ...
    if (data.borderColor !== undefined) {
      ...
      values.push(data.borderColor);
    }
    if (data.borderWidth !== undefined) {
      updates.push('border_width = ?');
      values.push(data.borderWidth);
    }
    if (data.smoothing !== undefined) {
      updates.push('smoothing = ?');
      values.push(data.smoothing);
    }
