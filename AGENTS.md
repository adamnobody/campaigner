...
## Agentmemory / MCP memory

Если агенту доступны инструменты `agentmemory` / MCP memory, использовать их
дисциплинированно: память помогает восстановить долгосрочный контекст, но не
заменяет чтение файлов, `git status`, `git log`, `rg` и локальный анализ кода.

В начале каждой сессии агент обязан восстановить контекст через доступные
memory-инструменты, например:
- `memory_recall`
- `memory_smart_search`
- `memory_sessions`
- `memory_file_history`
...
Минимальный стартовый контекст для поиска:
- репозиторий `campaigner`
- текущая ветка
- текущий фокус roadmap / версии
- активные ADR
- последние архитектурные решения
- ограничения пользователя и процесса

Записывать в долгосрочную память можно только устойчивые факты, которые будут
полезны в следующих сессиях:

- архитектурные решения и их статус;
- ограничения пользователя и процесса;
- важные договорённости по веткам, workflow и forbidden actions;
- структура проекта;
- принятые naming/type conventions;
- связь между документацией и кодом;
- результаты значимых проверок, если они подтверждают архитектурное или
  процессное состояние;
- важные выводы из ADR, SPEC, REPORT или git-истории.

Не сохранять в memory:

- временные логи;
- одноразовые команды;
- случайные промежуточные выводы;
- содержимое больших файлов без необходимости;
- черновики;
- очевидные факты, которые легко получить из файлов;
- непроверенные предположения;
- секреты, токены, приватные ключи, credentials и персональные данные.

Перед записью в память агент должен формулировать запись кратко, проверяемо и
полезно для будущих сессий. Если есть сомнение, является ли информация
долгосрочно полезной, не сохранять её.

`memory_save` использовать только после подтверждения факта одним из источников:

- кодом;
- документацией;
- ADR/SPEC/REPORT;
- git-историей;
- явным указанием пользователя.

`memory_checkpoint` использовать после завершения значимого этапа, если
инструмент доступен и это уместно.

В финальном отчёте агент должен указать:

- какие memory-инструменты использовались для восстановления контекста;
- какие факты были сохранены в memory, если сохранялись;
- если ничего не сохранялось — явно написать `memory: ничего не сохранялось`.

Memory не отменяет остальные правила `AGENTS.md`: агент всё равно обязан читать
релевантные файлы, проверять состояние репозитория и соблюдать минимальный
дифф.
