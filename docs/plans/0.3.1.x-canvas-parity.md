...
**Следующий шаг:** **PR3** — multi-ring reconciler + toolbar «Complete ring» (C3 vertex edit закрыт; multi-ring create — после PR3).  
...
| **Отложено (PR3)** | Toolbar «Complete ring» при `draw_territory` + reconciler всех `rings[]` вне edit-mode — иначе rings[1+] сохраняются, но не видны |
| **Esc / guards** | Esc отменяет edit; guard «finish shape edit first» при повторном входе |

**Тест:** шаг 6 — vertex edit на одном/нескольких кольцах в edit-overlay; multi-ring create — после PR3.
