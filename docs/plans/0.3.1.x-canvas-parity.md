...
**Статус:** план  
**Блокер:** закрытие 0.3.1  
**Код в этой задаче не менялся** — только этот документ.
...
- [ ] **B1** — восстановить `MapMarkerDialog` + `MapMarkerPanel` по HEAD, адаптировать под canvas API
...
- [ ] **B2** — emoji + подпись + badges (note / child scene)
...
- [ ] **B4** — nested maps CORE (зависит от **A1**)
...
- [ ] **C1** — режим рисования территории + save как territory
