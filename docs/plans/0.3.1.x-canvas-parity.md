...
**Статус:** **parity + acceptance smoke закрыты** на `testbed-rust` (`db4392890`)  
**Следующий шаг (2026-06-07):** product polish (inspectors, layers UI) и runtime debt — см. `docs/canvas-functional-gaps.md`, `docs/canvas-debt.md`.  
**Связанные закрытые вехи:** 0.3.2 architecture core ✅ (ADR-0004 Accepted, migration 018); 0.3.5 text parity ✅ (`d8b0f4664`).

---

## Контекст и цель

При переходе «Карта» → «Холст» (исторический аудит: git до `db4392890`) были удалены core-UI маркеров, территорий и вложенных карт. **Parity восстановлен** в подзаходах A–D ниже.
...
**0.3.1 закрывается когда все 6 шагов проходят** (ручной smoke на desktop Tauri) **и** automated gates ниже — **PASS** на `testbed-rust` **2026-06-04**.

### Acceptance smoke (6 шагов)

| # | Сценарий | Результат | Верификация (сессия `db4392890`) |
|---|----------|-----------|----------------------------------|
| 1 | Маркер: title, icon, color, заметка → icon + label на canvas | **PASS** | `canvasReconciler.drawMarker` (emoji + `Text`); `MapMarkerDialog` + `buildMarkerCreateInput`; B1/B2 |
| 2 | Переоткрыть маркер → правки видны | **PASS** | `MapMarkerPanel` → `openMarkerDialog` → `applyMarkerFormToObject` + `persistObject`; B3 |
| 3 | Вложенная карта → объект в child → breadcrumbs назад → маркер на месте | **PASS** | `canvas::tests::attach_child_scene_*`; `navigationStack.test.ts`; B4 (`attachChildSceneToMarker`, `MapToolbar` crumbs) |
| 4 | Территория + фракция + стили → label на canvas, цвет от фракции | **PASS** | `MapTerritoryDialog` (`factionId`, `color`/`borderColor` из faction); `territoryLabel.ts` + reconciler; C2/C4 |
| 5 | Страница state/faction → территория в списке | **PASS** | `canvas::tests::territory_kind_is_listed_and_synced_with_faction`; `list_territories_internal`; D2 + `factionNavigation.test.ts` |
| 6 | Vertex edit + второй контур (multi-ring) | **PASS** | `territoryGeometry.test.ts`, `territoryDrawing.test.ts`, `geometryRings.test.ts`; C3 toolbar Complete ring + edit session |

**Примечание:** интерактивный click-through в Tauri desktop в agent-сессии не выполнялся (нет UI automation). Шаги закрыты по **code trace + unit/Rust tests**; перед merge в main рекомендуется **один** короткий visual pass оператором по таблице выше.

### Automated gates (2026-06-04, `testbed-rust` @ `db4392890`)

| Команда | Результат |
|---------|-----------|
| `npm run build:shared` | OK |
| `npm run build --workspace=frontend` | OK |
| `cargo test` (39 tests, incl. `attach_child_scene_*`, `territory_kind_is_listed_and_synced_with_faction`) | OK |
| Vitest parity (7 files, 35 tests: navigation, territory form/geometry/drawing/label, geometry rings, faction nav) | OK |
| `cargo clippy -- -D warnings` | не запускался в этой сессии (Rust не менялся) |
| `npm run tauri:codegen` | не требовался (DTO/commands не менялись) |

### Исходные формулировки шагов
...
*Последнее обновление: acceptance smoke 0.3.1 — 2026-06-04 (`db4392890`).*
