...
**Следующий шаг:** toolbar «Complete ring» при `draw_territory` (PR3 reconciler закрыт; C4 labels отдельно).  
...
- [x] **C3** — vertex edit (частично: multi-ring toolbar отложен до PR3)

| | |
|--|--|
| **Эталон** | `HEAD:useMapTerritoryCrud.ts`, `useMapTerritoryDrawing.ts`, `useMapInteractions.ts` (insert/delete vertex, edit shape mode) |
| **Pixi** | `PixiMapCanvas` overlay handles; `territoryGeometry.ts`; persist `geometry_json.rings` через `reconcileScene` |
| **Сделано** | Drag вершин; dbl-click ребро → insert; right-click вершина → delete (≥3); Save/Cancel toolbar; Esc; overlay рисует **все** rings в edit-mode |
| **Отложено (post-PR3)** | Toolbar «Complete ring» при `draw_territory` — разблокирован после PR3 reconciler |
| **Esc / guards** | Esc отменяет edit; guard «finish shape edit first» при повторном входе |

**Тест:** шаг 6 — vertex edit на одном/нескольких кольцах; multi-ring create — после toolbar Complete ring.
...
| **Не в scope** | Complete ring toolbar (следующий шаг); smoothing в Pixi; holes/islands semantics |
