...
export function MarkerDialog(props: {
  ...
  // [NEW] Пропс для предустановки типа (нужен для полигонов)
  ...
  // [MODIFIED] Учитываем initialType при инициализации
  const [markerType, setMarkerType] = useState<MarkerType>(
    (initial?.marker_type as MarkerType) ?? initialType ?? 'location'
  );
  ...
  React.useEffect(() => {
    ...
    // [MODIFIED] Сброс типа при открытии
    setMarkerType((initial?.marker_type as MarkerType) ?? initialType ?? 'location');
    ...
    setNewMapTitle('');
    setNewMapParent(defaultParentMapId ?? '');
    setNewMapFile(null);
  }, [open, initial, initialType, defaultParentMapId]);
  ...
  const linkPayload = useMemo<MarkerLinkPayload>(() => {
    ...
    if (linkType === 'note') {
      return { link_type: 'note', link_note_id: linkNoteId || null, link_map_id: null };
    }
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
        ...
        <DialogContent sx={{ pt: 1 }}>
          ...
          >
            ...
            {/* [NEW] Добавили Зону */}
            <MenuItem value="area">Зона / Область</MenuItem>
          </TextField>
          ...
          <TextField
            ...
            InputProps={{
              endAdornment: (
                <input
                  type="color"
                  value={/^#[0-9a-fA-F]{6}$/.test(color) ? color : '#ff4757'}
                  onChange={(e) => setColor(e.target.value)}
          ...
          <TextField
            ...
            label="Тип ссылки"
            value={linkType}
            onChange={(e) => setLinkType(e.target.value as LinkTypeUI)}
            margin="normal"
          >
          ...
          {linkType === 'note' && (
            <>
              <TextField
                select
                fullWidth
                label="Выберите заметку"
                value={linkNoteId}
                onChange={(e) => setLinkNoteId(e.target.value)}
                margin="normal"
              >
                {notes.length === 0 ? (
                  <MenuItem value="" disabled>Нет заметок</MenuItem>
                ) : (
                  notes.map(n => <MenuItem key={n.id} value={n.id}>{n.title}</MenuItem>)
                )}
              </TextField>

              <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                <Button variant="outlined" onClick={() => setCreateNoteOpen(true)}>
                  Создать заметку и привязать
                </Button>
              </Box>
            </>
          )}
          ...
          {linkType === 'map' && (
            <>
              ...
              >
                {maps.length === 0 ? (
                  <MenuItem value="" disabled>Нет карт</MenuItem>
                ) : (
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                <Button variant="outlined" onClick={() => setCreateMapOpen(true)}>
                  Создать карту и привязать
                </Button>
              </Box>
            </>
          )}
        ...
        <DialogActions>
          ...
          {mode === 'create' ? (
            <Button
              variant="contained"
              disabled={!canSubmit || !linkValid || !initial || !onCreate}
              onClick={async () => {
            ...
            <Button
              variant="contained"
              disabled={!canSubmit || !linkValid || !onSave}
              onClick={async () => {
                if (!onSave) return;
                await onSave({
                  title: title.trim(),
      ...
      {/* Mini dialogs (Note/Map) остались без изменений */}
      ...
      <Dialog open={createMapOpen} onClose={() => setCreateMapOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent sx={{ pt: 1 }}>
          ...
          <TextField
            ...
            value={newMapParent}
            onChange={(e) => setNewMapParent(e.target.value)}
            margin="normal"
          >
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            Файл: {newMapFile ? newMapFile.name : 'не выбран'} (лимит 40MB)
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              const created = await onCreateLinkedMap({
                title: newMapTitle.trim(),
                ...(newMapParent ? { parent_map_id: newMapParent } : {}),
                file: newMapFile
              });
              setLinkType('map');
              setLinkMapId(created.id);
              setCreateMapOpen(false);
            }}
          >
            Создать и выбрать
