...
export async function listProjects(): Promise<ProjectDTO[]> {
  ...
  const rows = db.prepare('SELECT id, name, path, system, created_at FROM app_projects ORDER BY created_at DESC').all();
  return rows as ProjectDTO[];
}

export async function getProjectById(projectId: string): Promise<ProjectDTO | null> {
  const db = await getAppDb();
  const row = db.prepare('SELECT id, name, path, system, created_at FROM app_projects WHERE id = ?').get(projectId);
...
export async function createProject(input: { name: string; rootPath?: string; system: string }): Promise<ProjectDTO> {
  ...
  const root = input.rootPath
    ? (path.isAbsolute(input.rootPath) ? input.rootPath : (() => { throw Object.assign(new Error('rootPath must be absolute.'), { status: 400, code: 'ROOT_NOT_ABSOLUTE' }); })())
  ...
  appDb.prepare('INSERT INTO app_projects (id, name, path, system, created_at) VALUES (?, ?, ?, ?, ?)')
