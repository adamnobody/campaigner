...
- `frontend/src/api/axiosClient.ts` — compatibility-слой для legacy, не primary entrypoint для нового кода.
...
- Для нового frontend API-кода не использовать `axiosClient.ts` как основной путь импорта.
...
- `frontend/src/pages/MapPage.tsx` рендерит обычный `<img>` как подложку;
...
- `frontend/src/api/axiosClient.ts` — deprecated compatibility layer; canonical путь: `frontend/src/api/client.ts` + `frontend/src/api/<domain>.ts`.
...
- domain-local подкомпоненты в `frontend/src/pages/<domain>/*` (диалоги, утилиты, хуки), без преждевременного выноса в глобальные `components`.
...
Что видно во frontend (`frontend/src/pages/DogmasPage.tsx` + `pages/dogma/*`):
...
Страница фракции (`frontend/src/pages/FactionDetailPage.tsx`) организована табами `EntityTabs`:
...
- **ReactFlow реально используется** в `frontend/src/pages/dynasty/FamilyTree.tsx` для интерактивного семейного дерева династии (узлы, связи, dagre layout, drag, сохранение координат);
...
Основная страница кастомизации — `/appearance` (`frontend/src/pages/AppearanceSettingsPage.tsx`).
...
Реализация:
- страница `frontend/src/pages/WikiGraphPage.tsx`;
